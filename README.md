...
If every one of the `MAX_STALL_RETRIES` attempts stalls in a row (as opposed to a real error like a bad token, which is never retried this way), that's taken as a strong signal the Xet endpoint itself is the problem — the downloader prints a heads-up and automatically switches to the classic HTTP/LFS path (`HF_HUB_DISABLE_XET=1`) for one more fresh set of attempts before giving up. No manual intervention needed anymore; you can still set `HF_HUB_DISABLE_XET=1` yourself ahead of time to skip straight to the classic path.
