...
""",
    ...
    pull_parser.add_argument(
        "--no-token",
        action="store_true",
        help="With --dest: ignore HF_TOKEN even if it's set and download "
        "unauthenticated. HuggingFace tracks per-token and per-IP rate "
        "limits separately, so a token that's seen heavy traffic can "
        "stall while unauthenticated access to the same repo works fine "
        "— worth trying if a transfer stalls repeatedly despite retries. "
        "Prints a heads-up naming the ignored token (masked) when this "
        "applies. Default: off (use HF_TOKEN when present).",
    )
